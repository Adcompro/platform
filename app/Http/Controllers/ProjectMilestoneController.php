...
class ProjectMilestoneController extends Controller
{
    ...
    public function update(Request $request, Project $project, ProjectMilestone $milestone)
    {
        ...
        try {
            ...
            // Track old values
            $oldValues = $milestone->only(['name', 'status', 'fixed_price', 'pricing_type']);
            ...
            // Log changes
            $newValues = $milestone->only(['name', 'status', 'fixed_price', 'pricing_type']);
            $changes = [];
            ...
            if (!empty($changes)) {
                foreach ($changes as $fieldName => $fieldData) {
                    $description = 'updated milestone "' . $milestone->name . '" - ' . str_replace('_', ' ', $fieldName);
                    ProjectActivity::log(
                        $project->id,
                        'milestone_updated',
                        $description,
    ...
    public function destroy(Request $request, Project $project, ProjectMilestone $milestone)
    {
        ...
        try {
            ...
            // Log milestone deletion activity
            $milestoneName = $milestone->name;
            ProjectActivity::log(
                $project->id,
                'milestone_deleted',
                'deleted milestone "' . $milestoneName . '"' . ($cascadeDelete ? ' (with ' . $taskCount . ' tasks and ' . $subtaskCount . ' subtasks)' : ''),
                null,
                'milestone',
                $milestone->id
            );
