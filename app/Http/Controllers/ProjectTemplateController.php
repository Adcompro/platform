...
class ProjectTemplateController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Super admin sees all templates, others see only their company's templates
        if ($user->role !== 'super_admin') {
            // Non-super admins only see their company's templates
            if (Schema::hasColumn('project_templates', 'company_id')) {
                $query->where('company_id', $user->company_id);
            }
        }

        // Apply company filter if specified (for super_admin to filter)
