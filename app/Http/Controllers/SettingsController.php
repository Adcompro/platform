...
class SettingsController extends Controller
{
    ...
    public function index()
    {
        ...
        $settings = [
            ...
            // Teamleader OAuth settings
            'teamleader_client_id' => Setting::get('teamleader_client_id', ''),
            'teamleader_client_secret' => Setting::get('teamleader_client_secret', ''),
            'teamleader_redirect_uri' => Setting::get('teamleader_redirect_uri', url('/settings/teamleader/oauth/callback')),
    ...
    public function update(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            // Teamleader OAuth validation
            'teamleader_client_id' => 'nullable|string|max:255',
            'teamleader_client_secret' => 'nullable|string|max:255',
            'teamleader_redirect_uri' => 'required|url|max:255',
        ...
        // Update Teamleader OAuth settings
        Setting::set('teamleader_client_id', $validated['teamleader_client_id']);
        Setting::set('teamleader_client_secret', $validated['teamleader_client_secret']);
        Setting::set('teamleader_redirect_uri', $validated['teamleader_redirect_uri']);
    ...
    /**
     * Sync Time Entries from Teamleader
     */
    public function syncTimeEntries(Request $request)
    {
        // Authorization check
        if (!in_array(Auth::user()->role, ['super_admin', 'admin'])) {
            abort(403, 'Access denied. Only administrators can run sync operations.');
        }
        ...
        try {
            ...
            \Artisan::call('teamleader:sync-time-entries', [
                '--force' => true
            ]);

            $output = \Artisan::output();

            Log::info('Teamleader Time Entries Sync completed via web interface', [
                'user_id' => Auth::id(),
                'output' => $output
            ]);

            return redirect()->route('settings.index')
                ->with('success', 'Time Entries sync completed successfully! Check the statistics above for results.');

        } catch (\Exception $e) {
            Log::error('Teamleader Time Entries Sync failed via web interface', [
                'user_id' => Auth::id(),
                'error' => $e->getMessage()
            ]);

            return redirect()->route('settings.index')
                ->with('error', 'Time Entries sync failed: ' . $e->getMessage());
        }
    }

    /**
     * Run Full Master Sync from Teamleader
     */
    public function masterSync(Request $request)
    {
        // Authorization check
        if (!in_array(Auth::user()->role, ['super_admin', 'admin'])) {
            abort(403, 'Access denied. Only administrators can run sync operations.');
        ...
        try {
            ...
            // Run the sync command
            \Artisan::call('teamleader:master-sync', [
                '--force' => true
            ]);

            $output = \Artisan::output();

            Log::info('Teamleader Master Sync completed via web interface', [
                'user_id' => Auth::id(),
                'output' => $output
            ]);

            return redirect()->route('settings.index')
                ->with('success', 'Master Sync completed successfully! All Teamleader data has been synchronized to the cache.');

        } catch (\Exception $e) {
            Log::error('Teamleader Master Sync failed via web interface', [
                'user_id' => Auth::id(),
                'error' => $e->getMessage()
            ]);

            return redirect()->route('settings.index')
                ->with('error', 'Master Sync failed: ' . $e->getMessage());
        }
    }
}