...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Super admin ziet alle users, admin alleen van eigen company
        if ($user->role === 'admin') {
        ...
        // Company filtering (alleen voor super_admin)
        if ($request->filled('company_id') && $user->role === 'super_admin') {
            $query->where('company_id', $request->company_id);
        }

        $users = $query->orderBy('name')->paginate(20)->withQueryString();

        // Get companies voor filter dropdown (alleen voor super_admin en als plugin actief)
        $companies = collect();
        if ($user->role === 'super_admin' && $this->isCompaniesPluginActive()) {
            $companies = Company::where('is_active', true)->orderBy('name')->get();
        }

        // Calculate stats
        $stats = [
            'total_users' => User::when($user->role === 'admin', function($q) use ($user) {
                $q->where('company_id', $user->company_id);
            })->count(),
            'active_users' => User::when($user->role === 'admin', function($q) use ($user) {
                $q->where('company_id', $user->company_id);
            })->where('is_active', true)->count(),
            'inactive_users' => User::when($user->role === 'admin', function($q) use ($user) {
                $q->where('company_id', $user->company_id);
    ...
    public function create()
    {
        ...
        if ($this->isCompaniesPluginActive()) {
            if ($user->role === 'super_admin') {
                ...
                // Admin kan alleen users aanmaken voor eigen company
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $data = [
                ...
                'company_id' => $user->role === 'super_admin' ? $request->company_id : $user->company_id,
    ...
    public function edit(User $user)
    {
        ...
        if ($this->isCompaniesPluginActive()) {
            if ($currentUser->role === 'super_admin') {
                ...
                // Admin kan alleen binnen eigen company werken
    ...
    public function editModal(User $user)
    {
        ...
        // Companies (alleen voor super_admin)
        $companies = null;
        if ($currentUser->role === 'super_admin' && $this->isCompaniesPluginActive()) {
    ...
    public function update(Request $request, User $user)
    {
        ...
        // Company validation (alleen super_admin mag company wijzigen)
        if ($currentUser->role === 'super_admin') {
            $rules['company_id'] = 'required|exists:companies,id';
        }
        ...
        try {
            ...
            // Update company alleen voor super_admin
            if ($currentUser->role === 'super_admin') {
    ...
    public function destroy(User $user)
    {
        ...
        $hasActiveProjects = $user->projects()->where('status', 'active')->exists();
        $hasPendingTimeEntries = $user->timeEntries()->where('status', 'pending')->exists();
        
        if ($hasActiveProjects || $hasPendingTimeEntries) {
            return back()->with('error', 'Cannot delete user with active projects or pending time entries. Please deactivate the user instead.');
        }
