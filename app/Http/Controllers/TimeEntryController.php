...
class TimeEntryController extends Controller
{
    ...
    public function approvals(Request $request)
    {
        ...
        $users = \App\Models\User::where('role', '!=', 'super_admin') // Verberg super_admin users
    ...
    public function index(Request $request)
    {
        ...
        // Als we vanuit een project komen, toon alle entries zonder paginatie
        if ($request->filled('return_to_project')) {
            $timeEntries = $query->get();
        } else {
            // Anders gebruik normale paginatie
        ...
        $projects = Project::when($user->role === 'admin', function($q) use ($user) {
            ...
            ->when(!in_array($user->role, ['super_admin', 'admin']), function($q) use ($user) {
                // Reguliere users zien alleen assigned projecten
    ...
    public function editModal(TimeEntry $timeEntry)
    {
        ...
        // Get projects - Super_admin ziet alle projecten, admin ziet company projecten, anderen alleen assigned projecten
        // Completed projecten worden uitgefilterd
        $projects = Project::when($user->role === 'admin', function($q) use ($user) {
                // Admin ziet alle projecten in eigen company
                $q->where('company_id', $user->company_id);
    ...
    public function getWorkItems(Request $request, $project)
    {
        try {
            ...
            // Check dat user member is OF heeft privileged role (super_admin, admin, project_manager)
            $allowedRoles = ['super_admin', 'admin', 'project_manager'];
            $hasPrivilegedRole = in_array(Auth::user()->role, $allowedRoles);

            if (!$hasPrivilegedRole && !$project->users->contains(Auth::user())) {
                Log::warning('User not member of project and no privileged role', [
    ...
    {
        // Check authorization - alleen super_admin en admin
        if (!in_array(Auth::user()->role, ['super_admin', 'admin'])) {
     ...
     * Format: Date | Start Time | End Time | Minutes | User Name | Hourly Rate | Amount |
     *         Service Code | Customer | Milestone | Project | Project Code | Task | Billable | Invoiced
     */
    public function importExcel(Request $request)
    {
        ...
        try {
            ...
            // Start vanaf row 2 (row 1 is header)
            foreach ($worksheet->getRowIterator(2) as $row) {
                ...
                try {
                    ...
                    // Parse Excel data
                    $date = $cells[0] ?? null;  // Column A: Date
                    $startTime = $cells[1] ?? null;  // Column B: Start Time
                    $endTime = $cells[2] ?? null;  // Column C: End Time
                    $minutes = $cells[3] ?? null;  // Column D: Minutes
                    $userName = $cells[4] ?? null;  // Column E: User Name
                    $hourlyRate = $cells[5] ?? null;  // Column F: Hourly Rate
                    $amount = $cells[6] ?? null;  // Column G: Amount
                    $serviceCode = $cells[7] ?? null;  // Column H: Service Code
                    $customerName = $cells[8] ?? null;  // Column I: Customer
                    $milestoneName = $cells[9] ?? null;  // Column J: Milestone
                    $projectName = $cells[10] ?? null;  // Column K: Project
                    $projectCode = $cells[11] ?? null;  // Column L: Project Code
                    $taskName = $cells[12] ?? null;  // Column M: Task
                    $billable = $cells[13] ?? 'Ja';  // Column N: Billable
                    $invoiced = $cells[14] ?? 'Nee';  // Column O: Invoiced

                    // Valideer verplichte velden
                    if (!$date || !$customerName || !$projectName || !$userName || !$minutes) {
                        $errors[] = "Row $rowIndex: Missing required fields (date, customer, project, user, or minutes)";
                        $skipped++;
                        continue;
                    }

                    // Parse date (Excel date format)
                    if (is_numeric($date)) {
                    ...
                    $project = $this->findOrCreateProject($projectName, $customer, $projectCode);

                    // Find or create milestone
                    $milestone = $this->findOrCreateMilestone($project, $milestoneName);

                    // Find or create task (if provided)
                    $task = null;
                    if ($taskName && $milestone) {
                        $task = $this->findOrCreateTask($milestone, $taskName);
                    }

                    // Find user by name
                    $user = User::where('name', 'like', '%' . $userName . '%')->first();
                    if (!$user) {
                        $errors[] = "Row $rowIndex: User not found: $userName";
                        $skipped++;
                        continue;
                    }
                    ...
                    TimeEntry::create([
                        ...
                        'project_milestone_id' => $milestone ? $milestone->id : null,
                        'project_task_id' => $task ? $task->id : null,
                        'project_subtask_id' => null,
                        'entry_date' => $date->format('Y-m-d'),
                        'minutes' => intval($minutes),
                        'hours' => round(intval($minutes) / 60, 2),
                        'description' => $serviceCode . ($taskName ? ': ' . $taskName : ''),
                        'is_billable' => (strtolower($billable) === 'ja' || strtolower($billable) === 'yes') ? 'billable' : 'non_billable',
                        'status' => 'approved', // Auto-approve imports
                        'approved_by' => Auth::id(),
                        'approved_at' => now(),
                        'hourly_rate_used' => $hourlyRate ? floatval($hourlyRate) : null,
                        'is_invoiced' => (strtolower($invoiced) === 'ja' || strtolower($invoiced) === 'yes'),
                        'created_by' => Auth::id(),
                        'updated_by' => Auth::id(),
                    ]);
                    ...
                    Log::error('Error importing row ' . $rowIndex, [
                        'error' => $e->getMessage(),
                        'trace' => $e->getTraceAsString()
                    ]);
            ...
            $message = "Import completed: $imported time entries imported";
            if ($skipped > 0) {
                $message .= ", $skipped rows skipped";
            }

            if (!empty($errors)) {
                Log::warning('Import had errors', ['errors' => $errors]);
                return redirect()->route('time-entries.index')
                    ->with('warning', $message . '. Check logs for details.');
            }
     ...
     * Find or create customer
     */
    private function findOrCreateCustomer($customerName)
    {
        ...
        if (!$customer) {
            ...
            Log::info('Auto-created customer during import', [
                'customer_id' => $customer->id,
     ...
     * Find or create project
     */
    private function findOrCreateProject($projectName, $customer, $projectCode = null)
    {
        $query = Project::where('name', $projectName)
            ->where('company_id', Auth::user()->company_id);

        if ($projectCode) {
            $query->orWhere('project_code', $projectCode);
        }
        ...
        if (!$project) {
            ...
            Log::info('Auto-created project during import', [
                'project_id' => $project->id,
     ...
     * Find or create milestone
     */
    private function findOrCreateMilestone($project, $milestoneName)
    {
        if (!$milestoneName) {
            return null;
        }
        ...
        if (!$milestone) {
            $maxSortOrder = ProjectMilestone::where('project_id', $project->id)->max('sort_order') ?? 0;
            ...
            Log::info('Auto-created milestone during import', [
     ...
     * Find or create task
     */
    private function findOrCreateTask($milestone, $taskName)
    {
        if (!$taskName || !$milestone) {
            return null;
        }

        $task = ProjectTask::where('project_milestone_id', $milestone->id)
            ->where('name', $taskName)
            ->first();

        if (!$task) {
            $maxSortOrder = ProjectTask::where('project_milestone_id', $milestone->id)->max('sort_order') ?? 0;
            ...
            Log::info('Auto-created task during import', [
                'task_id' => $task->id,
                'task_name' => $taskName,
                'milestone_id' => $milestone->id
            ]);
