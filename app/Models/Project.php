...
{
    use HasFactory, SoftDeletes, HasStatusManagement;
    ...
    /**
     * Parent recurring project (als dit een automatisch gegenereerd recurring project is)
     */
    public function parentRecurringProject(): BelongsTo
    {
        return $this->belongsTo(Project::class, 'parent_recurring_project_id');
    }

    /**
     * Child recurring projects (alle automatisch gegenereerde projecten vanuit dit master project)
     */
    public function childRecurringProjects(): HasMany
    {
        return $this->hasMany(Project::class, 'parent_recurring_project_id');
    }

    /**
     * Activity log voor dit project
     */
    ...
    /**
     * Bereken totale kosten van gelogde tijd (in euro's)
     * Gebruikt hourly_rate_used uit time_entries
     */
    public function getTotalTimeCostsAttribute(): float
    {
        ...
        return $entries->sum(function($entry) {
            // Convert decimale hours + minutes naar totaal uren
            $totalMinutes = round(($entry->hours * 60) + $entry->minutes);
            $totalHours = $totalMinutes / 60;

            // Vermenigvuldig met hourly rate
            return $totalHours * ($entry->hourly_rate_used ?? 0);
        });
    }

    /**
     * Bereken totale gelogde uren (correcte berekening met minuten)
     */
