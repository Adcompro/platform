...
Route::middleware(['auth'])->group(function () {
    ...
    // Recurring Projects Dashboard (Project Manager en hoger)
    Route::get('/recurring-dashboard', [App\Http\Controllers\RecurringDashboardController::class, 'index'])->name('recurring-dashboard');
    Route::post('/recurring-dashboard/refresh', [App\Http\Controllers\RecurringDashboardController::class, 'refreshData'])->name('recurring-dashboard.refresh');
    ...
    Route::get('companies/create-modal', [CompanyController::class, 'createModal'])->name('companies.create-modal');
    ...
    Route::post('customers/{customer}/import-contacts', [CustomerController::class, 'importContactsFromTeamleader'])->name('customers.import-contacts');
    ...
    // USER MANAGEMENT
    // =====================================
    // BELANGRIJK: Custom routes MOETEN vóór Route::resource() staan!

    // User bulk action route - MUST be before Route::resource()
    ...
    // Resource route MOET als LAATSTE
    ...
    // Project bulk action route - MUST be before Route::resource()
    ...
    Route::prefix('projects/{project}')->group(function () {
        ...
        // Recurring settings update
        Route::put('update-recurring-settings', [ProjectController::class, 'updateRecurringSettings'])->name('projects.updateRecurringSettings');

        // Link to recurring master
        Route::post('link-to-recurring', [ProjectController::class, 'linkToRecurring'])->name('projects.linkToRecurring');

        // Update recurring series ID (for any project)
        Route::put('update-series-id', [ProjectController::class, 'updateSeriesId'])->name('projects.updateSeriesId');

        // Toggle master template status
        Route::post('toggle-master-template', [ProjectController::class, 'toggleMasterTemplate'])->name('projects.toggleMasterTemplate');
        ...
        // Time entries
        Route::get('time-entries', [ProjectController::class, 'getTimeEntries'])->name('projects.time-entries');
        Route::get('task-time-entries', [ProjectController::class, 'getTaskTimeEntries'])->name('projects.task-time-entries');
    ...
    // TIME ENTRIES IMPORT ROUTES (must be BEFORE resource routes!)
    ...
    // TIME ENTRIES EXPORT (before resource route!)
    Route::get('time-entries/export', [TimeEntryController::class, 'export'])->name('time-entries.export');

    // TIME ENTRIES APPROVAL ROUTES (before resource route!)
    ...
    // =====================================
    // PROJECT YEAR BUDGET OVERVIEW
    // =====================================
    Route::get('projects/{project}/year-budget', [ProjectController::class, 'yearBudget'])->name('projects.year-budget');
    Route::get('projects/{project}/series-budget', [ProjectController::class, 'seriesBudget'])->name('projects.series-budget');
    Route::post('projects/{project}/recalculate-year', [ProjectController::class, 'recalculateYear'])->name('projects.recalculate-year');
    ...
    // Route::get('settings/ai-usage', [SettingsController::class, 'aiUsage'])->name('settings.ai-usage'); // UITGESCHAKELD
    ...
    // Teamleader Sync Routes
    Route::post('settings/teamleader/sync-time-entries', [SettingsController::class, 'syncTimeEntries'])->name('settings.teamleader.sync-time-entries');
    Route::post('settings/teamleader/master-sync', [SettingsController::class, 'masterSync'])->name('settings.teamleader.master-sync');
...
Route::middleware(['auth'])->group(function () {
    ...
    // =====================================
    // TEAMLEADER FOCUS INTEGRATION 🔗 NEW! (21-10-2025) - Super Admin Only
    // =====================================
    Route::prefix('settings/teamleader')->name('teamleader.')->group(function () {
        Route::get('/', [TeamleaderController::class, 'index'])->name('index');
        Route::get('/authorize', [TeamleaderController::class, 'authorize'])->name('authorize');
        Route::get('/oauth/callback', [TeamleaderController::class, 'callback'])->name('callback');
        Route::post('/disconnect', [TeamleaderController::class, 'disconnect'])->name('disconnect');
        Route::get('/test-connection', [TeamleaderController::class, 'testConnection'])->name('test-connection');

        // Preview endpoints
        Route::get('/preview/companies', [TeamleaderController::class, 'previewCompanies'])->name('preview.companies');
        Route::get('/preview/contacts', [TeamleaderController::class, 'previewContacts'])->name('preview.contacts');
        Route::get('/preview/projects', [TeamleaderController::class, 'previewProjects'])->name('preview.projects');

        // Selection endpoints
        Route::get('/select/companies', [TeamleaderController::class, 'selectCompanies'])->name('select.companies');
        Route::get('/select/users', [TeamleaderController::class, 'selectUsers'])->name('select.users');
        Route::get('/select/projects', [TeamleaderController::class, 'selectProjects'])->name('select.projects');
        Route::get('/select/contacts', [TeamleaderController::class, 'selectContacts'])->name('select.contacts');

        // Sync endpoints (cache Teamleader data to local database)
        Route::post('/sync/projects', [TeamleaderController::class, 'syncProjectsForCustomer'])->name('sync.projects');
        Route::post('/sync/all', [TeamleaderController::class, 'syncAll'])->name('sync.all'); // Global sync ALL data
        Route::post('/sync/contact-companies', [TeamleaderController::class, 'syncContactCompanyRelationships'])->name('sync.contact-companies'); // Reverse sync contact-company relationships
        Route::get('/sync/status', [TeamleaderController::class, 'getSyncStatus'])->name('sync.status'); // Get sync progress status (AJAX)

        // Import endpoints
        Route::post('/import/companies', [TeamleaderController::class, 'importCompanies'])->name('import.companies');
        Route::post('/import/companies-with-contacts', [TeamleaderController::class, 'importCompaniesWithContacts'])->name('import.companies-with-contacts');
        Route::post('/import/users', [TeamleaderController::class, 'importUsers'])->name('import.users');
        Route::post('/import/projects', [TeamleaderController::class, 'importProjects'])->name('import.projects');
        Route::post('/import/contacts', [TeamleaderController::class, 'importContacts'])->name('import.contacts');
        Route::post('/import/time-tracking', [TeamleaderController::class, 'importTimeTracking'])->name('import.time-tracking');
    });
});
