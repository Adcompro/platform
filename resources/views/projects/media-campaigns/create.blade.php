...
@section('title', 'Create Media Campaign - ' . $project->name)
    ...
    <div class="bg-white/60 backdrop-blur-sm border-b border-slate-200/60">
        <div class="max-w-7xl mx-auto px-4 py-3">
            <div class="flex items-center space-x-3">
                ...
                <h1 class="text-2xl font-bold text-slate-800">Create Media Campaign</h1>
            </div>
        </div>
    </div>

    <div class="max-w-4xl mx-auto px-4 py-6">
        <form action="{{ route('projects.media-campaigns.store', $project->id) }}" method="POST" class="space-y-6">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="description" 
                                  ...
                                  rows="3"
                                  class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-500"
                                  style="">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-red-500" style="font-size: calc(var(--theme-font-size) - 2px);">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" 
                               ...
                               value="{{ old('press_release_date', now()->format('Y-m-d')) }}"
                               class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-500"
                               style=""
                               required>
                    ...
                    <div>
                        ...
                        <select id="campaign_type" 
                            ...
                            <option value="feature_announcement" {{ old('campaign_type') == 'feature_announcement' ? 'selected' : '' }}>Feature Announcement</option>
                            <option value="company_news" {{ old('campaign_type') == 'company_news' ? 'selected' : '' }}>Company News</option>
                            <option value="event" {{ old('campaign_type') == 'event' ? 'selected' : '' }}>Event</option>
                            <option value="partnership" {{ old('campaign_type') == 'partnership' ? 'selected' : '' }}>Partnership</option>
                            <option value="other" {{ old('campaign_type') == 'other' ? 'selected' : '' }}>Other</option>
                        </select>
                        @error('campaign_type')
                            <p class="mt-1 text-red-500" style="font-size: calc(var(--theme-font-size) - 2px);">{{ $message }}</p>
                        @enderror
            ...
            <div class="bg-white/60 backdrop-blur-sm border border-slate-200/60 rounded-xl p-6">
                ...
                <div>
                    <textarea id="notes" 
                              ...
                              class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-500"
                              style=""
                              placeholder="Any additional notes or context about this campaign...">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-red-500" style="font-size: calc(var(--theme-font-size) - 2px);">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex justify-end space-x-3">
                <a href="{{ route('projects.media-campaigns.index', $project->id) }}" 
