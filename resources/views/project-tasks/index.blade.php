                        ...
                        <ol class="inline-flex items-center space-x-1 md:space-x-3">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <span class="ml-1 text-gray-700 font-medium">Tasks</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                    <h1 class="mt-2 text-2xl font-bold text-gray-900">Tasks for {{ $projectMilestone->name }}</h1>
                </div>
                <div>
                    @if(in_array(Auth::user()->role, ['super_admin', 'admin', 'project_manager']))
                        <a href="{{ route('project-milestones.tasks.create', $projectMilestone) }}" 
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="text-lg font-medium text-gray-900">{{ $stats['total_tasks'] ?? 0 }}</dd>
                            </dl>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="text-lg font-medium text-gray-900">{{ $stats['in_progress_tasks'] ?? 0 }}</dd>
                            </dl>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="text-lg font-medium text-gray-900">{{ number_format($stats['total_estimated_hours'] ?? 0, 1) }}</dd>
                            </dl>
                        </div>
                    </div>
                </div>
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($tasks as $task)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ 
                                            $task->fee_type === 'in_fee' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'
                                        }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ number_format($task->estimated_hours ?? 0, 1) }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('project-milestones.tasks.show', [$projectMilestone, $task]) }}" 
                                            class="text-blue-600 hover:text-blue-900 mr-3">View</a>
                                        @if(in_array(Auth::user()->role, ['super_admin', 'admin', 'project_manager']))
                                            <a href="{{ route('project-milestones.tasks.edit', [$projectMilestone, $task]) }}" 
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Get started by creating a new task for this milestone.</p>
                    @if(in_array(Auth::user()->role, ['super_admin', 'admin', 'project_manager']))
                        <div class="mt-6">
                            <a href="{{ route('project-milestones.tasks.create', $projectMilestone) }}" 
                                class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ease-in-out duration-150">
