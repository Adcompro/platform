        ...
        <div style="padding: var(--theme-view-header-padding) 2rem; margin-left: 90px; max-width: calc(100vw - 8rem); padding-right: 3rem; height: 100%;">
            <div class="flex justify-between items-center" style="height: 100%;">
                ...
                <div class="flex space-x-2">
                    {{-- Show Deleted button - alleen voor admin en hoger --}}
                    @if(in_array(Auth::user()->role, ['super_admin', 'admin']))
                    <a href="{{ $showTrashed ? route('companies.index') : route('companies.index', ['show_trashed' => '1']) }}"
                       class="header-btn inline-flex items-center"
                       style="padding: calc(var(--theme-view-header-padding) * 0.5) var(--theme-view-header-padding); font-size: var(--theme-view-header-button-size); border: 1px solid {{ $showTrashed ? '#ef4444' : '#e2e8f0' }}; background-color: {{ $showTrashed ? 'rgba(239, 68, 68, 0.1)' : 'white' }}; color: {{ $showTrashed ? '#ef4444' : 'var(--theme-text)' }}; border-radius: 0.5rem;">
                    ...
                    <button type="button" onclick="openCreateCompanyModal()" id="header-create-btn"
                       class="header-btn"
                       style="padding: calc(var(--theme-view-header-padding) * 0.5) var(--theme-view-header-padding); font-size: var(--theme-view-header-button-size); cursor: pointer;">
    ...
    <div style="padding: 1.5rem 2rem;">
        ...
        <div class="bg-white border rounded-lg overflow-hidden" style="border-color: rgba(203, 213, 225, 0.6); box-shadow: var(--theme-card-shadow);">
                        ...
                        <tr class="transition-colors company-row table-row"
                            ...
                            <td style="padding: var(--theme-table-padding-y) var(--theme-table-padding-x);">
                                <div class="flex items-center justify-center space-x-2">
                                    {{-- View button - altijd zichtbaar voor iedereen --}}
                                    ...
                                    @if(in_array(Auth::user()->role, ['super_admin', 'admin']))
                                    @if($showTrashed)
                                        {{-- Restore button (voor deleted companies) --}}
                                        <form method="POST" action="{{ route('companies.restore', $company->id) }}" class="inline-block">
                                            @csrf
                                            <button type="submit"
                                                    class="p-1 hover:bg-green-50 rounded transition-colors"
                                        ...
                                        {{-- Force Delete button (alleen super_admin) --}}
                                        @if(Auth::user()->role === 'super_admin')
                                        <form method="POST" action="{{ route('companies.force-delete', $company->id) }}" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                    class="p-1 hover:bg-red-50 rounded transition-colors"
                                                    title="Permanently Delete Company"
                                                    onclick="return confirm('⚠️ WARNING: This will PERMANENTLY delete this company!\n\nThis action CANNOT be undone!\n\nAre you absolutely sure?')">
                                        ...
                                        @endif
                                    @else
                                        {{-- Edit button (voor active companies) --}}
                                        ...
                                        {{-- Soft Delete button (admin/super_admin) --}}
                                        <form method="POST" action="{{ route('companies.destroy', $company) }}" class="inline-block">
                                            ...
                                            <button type="submit"
                                                    ...
                                                    onclick="return confirm('Are you sure you want to delete this company?\n\nThis will move it to trash. You can restore it later from the trash view.')">
            ...
            <div class="px-6 py-12 text-center">
                ...
                @if(in_array(Auth::user()->role, ['super_admin', 'admin']))
                <div class="mt-6">
                    <button type="button" onclick="openCreateCompanyModal()" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">
...
<div id="edit-company-modal" class="fixed inset-0 z-50 hidden" style="background-color: rgba(0, 0, 0, 0.15);">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white border border-slate-200/40 rounded-xl overflow-hidden shadow-xl" style="max-width: 1200px; width: 100%; max-height: 98vh; overflow-y: auto;">
            {{-- Modal Header --}}
            <div class="border-b" style="border-color: rgba(203, 213, 225, 0.3); padding: 0.75rem 1.5rem;">
                <div class="flex justify-between items-center">
...
<div id="create-company-modal" class="fixed inset-0 z-50 hidden" style="background-color: rgba(0, 0, 0, 0.15);">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white border border-slate-200/40 rounded-xl overflow-hidden shadow-xl" style="max-width: 1200px; width: 100%; max-height: 98vh; overflow-y: auto;">
            {{-- Modal Header --}}
            <div class="border-b" style="border-color: rgba(203, 213, 225, 0.3); padding: 0.75rem 1.5rem;">
                <div class="flex justify-between items-center">
                    <h2 style="font-size: calc(var(--theme-font-size) + 2px); font-weight: 600; color: var(--theme-text); margin: 0;">Create New Company</h2>
                    <button type="button" onclick="closeCreateCompanyModal()" style="color: var(--theme-text-muted); background: none; border: none; font-size: 1.5rem; cursor: pointer; padding: 0.25rem;">
                        ×
                    </button>
                </div>
            </div>
            ...
            {{-- Modal Content --}}
            <div style="padding: 1rem 1.5rem;">
                <div id="create-company-modal-loading" class="text-center py-6">
                    <svg class="animate-spin h-6 w-6 mx-auto mb-3" style="color: var(--theme-primary);" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <p style="color: var(--theme-text-muted); font-size: calc(var(--theme-font-size) - 1px);">Loading create form...</p>
                </div>

                <div id="create-company-modal-content" class="hidden">
                    {{-- Content will be loaded here via AJAX --}}
                </div>
            </div>
        </div>
...
<script>
    ...
    function openCreateCompanyModal() {
        ...
        const modalLoading = document.getElementById('create-company-modal-loading');
        const modalContent = document.getElementById('create-company-modal-content');
        ...
        fetch('/companies/create-modal')
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Initialize form functionality for create
                    initializeCreateCompanyModalForm();
                } else {
                    throw new Error(data.message || 'Failed to load create form');
                }
            })
            .catch(error => {
                ...
                modalContent.innerHTML = `
                    <div class="text-center py-8">
                        <p style="color: var(--theme-danger);">Failed to load create form. Please try again.</p>
                        <button type="button" onclick="closeCreateCompanyModal()" style="margin-top: 1rem; padding: 0.5rem 1rem; background-color: var(--theme-primary); color: white; border: none; border-radius: var(--theme-border-radius);">
                            Close
                        </button>
                    </div>
                `;
    ...
    // Initialize create company modal form functionality
    function initializeCreateCompanyModalForm() {
        // Handle form submission
        const form = document.querySelector('#create-company-modal-content form');
        if (form) {
            form.addEventListener('submit', function(e) {
                e.preventDefault();
                submitCreateCompanyModalForm(form);
            });
        }

        // Style form elements according to theme
        styleCreateCompanyModalElements();
    }

    // Submit create company modal form
    function submitCreateCompanyModalForm(form) {
        ...
        .catch(error => {
            ...
            // On successful creation, Laravel might redirect, which causes this error
            // So we refresh the page to show the new company
            closeCreateCompanyModal();
            location.reload();
        })
        .finally(() => {
    ...
    // Style create company modal elements according to theme
    function styleCreateCompanyModalElements() {
        const primaryColor = getComputedStyle(document.documentElement).getPropertyValue('--theme-primary').trim();

        // Style submit buttons
        const submitButtons = document.querySelectorAll('#create-company-modal-content button[type="submit"]');
        ...
        const inputElements = document.querySelectorAll('#create-company-modal-content input[type="radio"], #create-company-modal-content input[type="checkbox"]');
