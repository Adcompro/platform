...
<div class="min-h-screen bg-gray-50">
                        ...
                        <ol class="inline-flex items-center space-x-1 md:space-x-3">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <span class="ml-1 text-gray-700 font-medium">Edit: {{ $subtask->name }}</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                    <h1 class="mt-2 text-2xl font-bold text-gray-900">Edit Subtask</h1>
                    <p class="mt-1 text-sm text-gray-600">Update subtask information for: {{ $subtask->name }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form action="{{ route('project-tasks.subtasks.update', [$projectTask, $subtask]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white shadow rounded-lg">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $subtask->name) }}" required
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">{{ old('description', $subtask->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                        <div>
                            ...
                            <select name="status" id="status" required
                                ...
                                <option value="on_hold" {{ old('status', $subtask->status) === 'on_hold' ? 'selected' : '' }}>On Hold</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="date" name="start_date" id="start_date" 
                                value="{{ old('start_date', $subtask->start_date ? $subtask->start_date->format('Y-m-d') : '') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                            @error('start_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="date" name="end_date" id="end_date" 
                                value="{{ old('end_date', $subtask->end_date ? $subtask->end_date->format('Y-m-d') : '') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                            @error('end_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <select name="fee_type" id="fee_type" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                                <option value="in_fee" {{ old('fee_type', $subtask->fee_type) === 'in_fee' ? 'selected' : '' }}>In Fee</option>
                                <option value="extended" {{ old('fee_type', $subtask->fee_type) === 'extended' ? 'selected' : '' }}>Extended</option>
                            </select>
                            @error('fee_type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="pricing_type" id="pricing_type" required
                                ...
                                <option value="hourly_rate" {{ old('pricing_type', $subtask->pricing_type) === 'hourly_rate' ? 'selected' : '' }}>Hourly Rate</option>
                                <option value="fixed_price" {{ old('pricing_type', $subtask->pricing_type) === 'fixed_price' ? 'selected' : '' }}>Fixed Price</option>
                            </select>
                            @error('pricing_type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        {{-- Fixed Price --}}
                        <div id="fixed_price_field" style="display: {{ old('pricing_type', $subtask->pricing_type) === 'fixed_price' ? 'block' : 'none' }}">
                            ...
                            <input type="number" name="fixed_price" id="fixed_price" step="0.01" min="0" 
                                value="{{ old('fixed_price', $subtask->fixed_price) }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                            @error('fixed_price')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div id="hourly_rate_field" style="display: {{ old('pricing_type', $subtask->pricing_type) === 'hourly_rate' ? 'block' : 'none' }}">
                            ...
                            <input type="number" name="hourly_rate_override" id="hourly_rate_override" step="0.01" min="0" 
                                value="{{ old('hourly_rate_override', $subtask->hourly_rate_override) }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="sort_order" id="sort_order" min="0" 
                                value="{{ old('sort_order', $subtask->sort_order) }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                            @error('sort_order')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                ...
                <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-between">
                    <button type="button" onclick="if(confirm('Are you sure you want to delete this subtask?')) { document.getElementById('delete-form').submit(); }"
        ...
        {{-- Delete Form --}}
        <form id="delete-form" action="{{ route('project-tasks.subtasks.destroy', [$projectTask, $subtask]) }}" method="POST" style="display: none;">
            @csrf
            @method('DELETE')
        </form>
    </div>
