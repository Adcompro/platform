                        ...
                        <ol class="inline-flex items-center space-x-1 md:space-x-3">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <span class="ml-1 text-gray-700 font-medium">{{ $milestone->name }}</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                    <h1 class="mt-2 text-2xl font-bold text-gray-900">{{ $milestone->name }}</h1>
                    <div class="mt-1 flex items-center space-x-3">
                        <span class="px-2.5 py-0.5 rounded-full text-xs font-medium {{ 
                            $milestone->status === 'completed' ? 'bg-green-100 text-green-800' : 
                            ($milestone->status === 'in_progress' ? 'bg-blue-100 text-blue-800' : 
                            ($milestone->status === 'on_hold' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'))
                        }}">
                            {{ ucfirst(str_replace('_', ' ', $milestone->status)) }}
                        </span>
                        <span class="px-2.5 py-0.5 rounded-full text-xs font-medium {{ $milestone->fee_type === 'in_fee' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800' }}">
                ...
                <div class="flex space-x-3">
                    ...
                    @if(in_array(Auth::user()->role, ['super_admin', 'admin']))
                        @php
                            ...
                            $subtaskCount = 0;
                            // Subtasks not used anymore
                        @endphp
                        <button type="button" onclick="openDeleteModal({{ $taskCount }}, {{ $subtaskCount }})" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 focus:bg-red-700 active:bg-red-800 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div class="lg:col-span-2">
                <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
                    <div class="border-b border-gray-100 mb-4 pb-3">
                        <h2 class="text-[17px] font-semibold">Milestone Information</h2>
                    </div>
                    <div>
                        <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                            @if($milestone->description)
                                <div class="sm:col-span-2">
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">{{ $milestone->description }}</dd>
                                </div>
                            @endif
                            
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Start Date</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ $milestone->start_date ? $milestone->start_date->format('M d, Y') : 'Not set' }}</dd>
                            </div>
                            
                            <div>
                                <dt class="text-sm font-medium text-gray-500">End Date</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ $milestone->end_date ? $milestone->end_date->format('M d, Y') : 'Not set' }}</dd>
                            </div>
                            
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Estimated Hours</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ number_format($milestone->estimated_hours ?? 0, 1) }} hours</dd>
                            </div>
                            
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Invoicing Trigger</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ ucfirst($milestone->invoicing_trigger) }}</dd>
                            </div>
                            
                            @if($milestone->deliverables)
                                <div class="sm:col-span-2">
                                    <dt class="text-sm font-medium text-gray-500">Deliverables</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ $milestone->deliverables }}</dd>
                                </div>
                            @endif
                        </dl>
                    </div>
            ...
            <div>
                <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
                    ...
                    <div class="space-y-2">
                        <a href="{{ route('project-milestones.tasks.index', $milestone) }}" class="flex items-center text-gray-600 hover:text-gray-900 transition-colors duration-200 group">
                            ...
                            <span class="text-sm">Manage Tasks</span>
                        </a>
                        
                        @if(in_array(Auth::user()->role, ['super_admin', 'admin', 'project_manager']))
                            <a href="{{ route('project-milestones.tasks.create', $milestone) }}" class="flex items-center text-gray-600 hover:text-gray-900 transition-colors duration-200 group">
                                <svg class="w-4 h-4 mr-2 text-gray-400 group-hover:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="mt-6 bg-white border border-gray-200 rounded-lg shadow-sm p-6">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between py-2 px-3 rounded hover:bg-gray-50 transition-colors">
                            ...
                            <span class="text-sm text-gray-900 font-medium">{{ $stats['total_tasks'] }}</span>
                        </div>
                        <div class="flex items-center justify-between py-2 px-3 rounded hover:bg-gray-50 transition-colors">
                            <span class="text-sm font-medium text-gray-500">Completed Tasks</span>
                            <span class="text-sm text-gray-900 font-medium">{{ $stats['completed_tasks'] }}</span>
                        </div>
                        <div class="flex items-center justify-between py-2 px-3 rounded hover:bg-gray-50 transition-colors">
        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
            <div class="border-b border-gray-100 mb-4 pb-3 flex items-center justify-between">
                <h2 class="text-[17px] font-semibold">Tasks</h2>
                <a href="{{ route('project-milestones.tasks.index', $milestone) }}" class="text-sm text-blue-600 hover:text-blue-500">View all →</a>
            </div>
            <div class="divide-y divide-gray-200">
                @forelse($milestone->tasks()->orderBy('sort_order')->limit(5)->get() as $task)
                    <div class="py-4">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                <h3 class="text-sm font-medium text-gray-900">
                                    <a href="{{ route('project-milestones.tasks.show', [$milestone, $task]) }}" class="hover:text-blue-600">{{ $task->name }}</a>
                                </h3>
                                <div class="mt-1 flex items-center text-xs text-gray-500">
                                    ...
                                    <span class="ml-2">0 subtasks</span>
                                    @if($task->estimated_hours)
                                        <span class="ml-2">{{ number_format($task->estimated_hours, 1) }} hours</span>
                                    @endif
                                </div>
                            </div>
                            <div class="ml-4 flex space-x-2">
                                <a href="{{ route('project-milestones.tasks.show', [$milestone, $task]) }}" class="text-gray-400 hover:text-gray-600">
                                ...
                                @if(in_array(Auth::user()->role, ['super_admin', 'admin', 'project_manager']))
                                    <a href="{{ route('project-milestones.tasks.edit', [$milestone, $task]) }}" class="text-gray-400 hover:text-gray-600">
                                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                        </svg>
                                    </a>
                ...
                @empty
                    <div class="py-6 text-center text-sm text-gray-500">
                        No tasks yet. 
                        @if(in_array(Auth::user()->role, ['super_admin', 'admin', 'project_manager']))
                            <a href="{{ route('project-milestones.tasks.create', $milestone) }}" class="text-blue-600 hover:text-blue-500">Create the first task →</a>
                        @endif
                    </div>
                @endforelse
            </div>
                            ...
                            <div class="mt-2">
                                ...
                                <div id="cascadeWarning" class="hidden mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                                    <p class="text-sm text-yellow-800">
                                        <strong>Warning:</strong> This milestone contains:
                                    </p>
                                    <ul class="mt-1 ml-4 text-sm text-yellow-700 list-disc">
                                        <li id="taskCountText"></li>
                                        <li id="subtaskCountText" class="hidden"></li>
                                    </ul>
                                    <div class="mt-3">
                                        <label class="flex items-center">
                                            ...
                                            <span class="ml-2 text-sm text-gray-700">Delete all tasks and subtasks</span>
                                        </label>
                                    </div>
                                </div>
...
<script>
function openDeleteModal(taskCount, subtaskCount) {
    ...
    if (taskCount > 0 || subtaskCount > 0) {
        ...
        taskCountText.textContent = taskCount + ' task(s)';
        if (subtaskCount > 0) {
            subtaskCountText.textContent = subtaskCount + ' subtask(s)';
            subtaskCountText.classList.remove('hidden');
        } else {
            subtaskCountText.classList.add('hidden');
        }
