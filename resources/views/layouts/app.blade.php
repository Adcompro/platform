            ...
            <div class="flex items-center justify-between px-6" style="height: var(--theme-header-height) !important; min-height: var(--theme-header-height) !important;">
                ...
                <nav class="flex-1 mx-8">
                    <div class="flex space-x-8 nav-{{ $themeSettings->top_nav_style ?: 'tabs' }}"
                        ...
                        <!-- Recurring Dashboard Tabs -->
                        @if(in_array(Auth::user()->role ?? '', ['super_admin', 'admin', 'project_manager']))
                        <div x-show="activeSection === 'recurring'" x-transition class="flex space-x-6">
                            <a href="{{ route('recurring-dashboard') }}"
                               class="tab-item @if(request()->routeIs('recurring-dashboard')) active @endif">
                                Monthly Overview
                            </a>
                        </div>
                        @endif
                        ...
                        <div x-show="activeSection === 'timetracking'" x-transition class="flex space-x-6">
                            ...
                            <a href="{{ route('time-entries.import.index') }}"
                               class="tab-item @if(request()->routeIs('time-entries.import.*')) active @endif">
                        ...
                        <div x-show="activeSection === 'invoices'" x-transition class="flex space-x-6">
                            <a href="{{ route('invoices.index') }}"
                               class="tab-item @if(request()->routeIs('invoices.*') && !request()->routeIs('invoice-templates.*')) active @endif">
                                Overview
                            </a>
                            {{-- Invoice Dashboard uitgeschakeld
                            @if(in_array(Auth::user()->role, ['super_admin', 'admin', 'project_manager']))
        ...
        <div class="flex-1 flex">
            ...
            <aside class="w-20 teamleader-sidebar border-r fixed top-0 bottom-0 left-0 z-50"
                ...
                <nav class="flex flex-col items-center space-y-2" style="background: transparent !important; padding-top: var(--theme-header-height) !important;">
                    ...
                    <!-- Budget Tracking Dashboard (Project Manager en hoger) -->
                    @if(in_array(Auth::user()->role ?? '', ['super_admin', 'admin', 'project_manager']))
                    <a href="{{ route('recurring-dashboard') }}"
                       @click="activeSection = 'recurring'"
                       :class="activeSection === 'recurring' ? 'active' : ''"
                       class="flex flex-col items-center justify-center w-16 h-16 rounded-lg px-1 py-2"
                       title="Budget Tracking Dashboard"
                       :style="activeSection === 'recurring' ? 'background-color: var(--theme-sidebar-active) !important; color: white !important;' : 'color: var(--theme-sidebar-text) !important; background: transparent !important;'">
                        <i class="fas fa-chart-line mb-1" style="font-size: var(--theme-sidebar-icon-size);"></i>
                        <span class="leading-tight text-center" style="font-size: var(--theme-sidebar-text-size);">Budget<br>Tracking</span>
                    </a>
                    @endif
